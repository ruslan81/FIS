	...
	$.widget("wijmo.wijtree", {
		...
		_createNodeWidget: function ($li, options) {
			if ($.fn.wijtreenode) {
				...
				if (!!options && $.isPlainObject(options)) {
					...
					$li.wijtreenode({ treeClass: this.widgetBaseClass });
				}
			}
			return $li;
		},
		...
		getCheckedNodes: function () {
			...
			$(self._nodeSelector(), self.element).each(function () {
				if ($(this).wijtreenode("option", "checked") &&
				$(this).wijtreenode("option", "checkState") !== "indeterminate") {
		...
		_setAllowDrag: function (value) {
			var $allNodes;
			if (value) {
				$allNodes = this.element.find(this._nodeSelector());
				$allNodes.each(function () {
					var w = $(this).data(this.widgetName);
				...
				$allNodes = this.element.find(this._nodeSelector() + ":ui-draggable");
	...
	$.widget("wijmo.wijtreenode", {
		...
		_changeText: function (text) {
			...
			if (self.$text !== null && text !== "") {
				self.$text.text(text);
				o.text = text;
				self._tree._trigger("nodeTextChanged", this);
			}
		},
		...
		_expandCollapseItem: function () {//access
			if (!this._tree.options.disabled) {
				if (this._hasChildren || this.options.hasChildren) {
		...
		_selectNode: function (select, event) {
			if (!this._tree.options.disabled && !this._tree._isDragging) {
				var self = this, ctrlKey = self._tree._ctrlKey, idx;
				if (ctrlKey) {
