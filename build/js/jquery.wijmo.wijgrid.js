...
(function ($) {
	...
	$.widget("wijmo.wijgrid", {
		options: {
			...
			//add by Jeffrey on Feb 14th 2011
			//add columnresizing and columnresized events
			/// <summary>
		...
		ensureControl: function (loadData, userData) {
			...
			if (this._initialized) {
				...
				this._prepareColumnOptions(); // prepare static columns
			}
		...
		doRefresh: function (userData) {
			...
			if (!this._initialized) {
				try {
					this._prepareColumnOptions(); // prepare static and dynamic columns
				}
				catch (e) {
					throw e;
				}
			...
			var dataSlice = this._dataStore.getDataSlice(),
				...
				leaves, ri, rowsLen, dataItem, newItem, i, len, leaf;
		...
		_prepareColumnOptions: function () {
			...
			if (this._dataStore.isLoaded()) {
				...
				$.each(leaves, function (i, leaf) {
					...
					if (autogenerationMode === "merge" || leaf.dynamic === true) { // assume headerText\ footerText options of the static columns only when using "merge" mode.
		...
		_handleSort: function (columnWidget, multiSort) {
			var columns = this.columns(),
				newSortDirection, args;

			if (this.options.allowSorting && ($.inArray(columnWidget, columns) >= 0)) {
				newSortDirection = ((columnWidget.options.sortDirection === "none")
					? "ascending"
					: ((columnWidget.options.sortDirection === "ascending") ? "descending" : "ascending"));

				args = { column: columnWidget.options, sortDirection: newSortDirection };
				...
				if (this._onColumnSorting(args)) {
					...
					if (multiSort) {
						...
						$.each(columns, function (index, item) {
							...
							if (item !== columnWidget && !(item.options.groupInfo && item.options.groupInfo.position !== "none")) {
								item.options.sortDirection = "none";
							}
						});

						// ensure invisible columns.
						$.wijmo.wijgrid.traverse(this.options.columns, function (item) {
							item.sortOrder = 0;

							if (item.travIdx !== columnWidget.options.travIdx && !(item.groupInfo && item.groupInfo.position !== "none")) {
		...
		_handleDragnDrop: function (dragTravIdx, dropTravIdx, at) {
			...
			if (this._onColumnDropping({ drag: drag.found, drop: drop.found, at: at })) {
				this.ensureControl(false, {
					...
					afterRefresh: function () { this._onColumnDropped({ drag: drag.found, drop: drop.found, at: at }); }
				});
			}
		},
		...
		_fieldResized: function (fieldWidget, oldWidth, newWidth) {
			...
			if (this._trigger("columnresizing", null, { oldWidth: oldWidth, newWidth: newWidth }) !== false) {
...
(function ($) {
	...
	$.widget("wijmo.c1basefield", {
		...
		_postset_allowMoving: function (value, oldValue) {
			if (value) {
	...
	$.widget("wijmo.c1field", $.wijmo.c1basefield, {
		...
		_prepareFilterCell: function () {
			...
			if (filterCellIndex >= 0) {
				...
				editorOptions = { culture: gridView.options.culture,
		...
		_onHrefClick: function (args) {
			...
			if (args.data.options.allowSort) {
				args.data._owner()._handleSort(args.data, args.ctrlKey);
			}
...
(function ($) {
	...
	$.extend($.wijmo.wijgrid, {
		...
		// 3 - tFoot
		// otherwise - table
		readTableSection: function (table, section) {
			...
			this.getDataSlice = function () {
				...
				if (!_transformed) {
					...
					if (_dataMode() !== $.wijmo.wijgrid.dataMode.dynamical) {
						_transformedData = _transform(_clonedItems);
					} else {
						_transformedData = {
			...
			function _domTableDataReader() {
				this.read = function (wijDataSource) {
					...
					if (wijDataSource && wijDataSource.data && wijDataSource.data.length) {
						wijDataSource.items = $.wijmo.wijgrid.readTableSection(wijDataSource.data, 2);
		...
		grouper: function () {
			...
			this._buildGroupRow = function (groupRange, cellRange, isHeader, isParentCollapsed) {
				var groupInfo = groupRange.owner,
					...
					headerOffset = 0,
					aggregate = "",
					tmp, cell, caption, args, span, col, agg;
		...
		dragAndDropHelper: function (wijgrid) {
			...
			this.attach = function (wijField) {
				...
				.draggable({
					...
					drag: function (e, ui) {
						if (_droppableWijField) {
							_showArrows(_droppableWijField, _getPosition(wijField, _droppableWijField, e, ui));
						}
					},

					start: function (e, ui) {
						if (wijgrid.options.allowColMoving && !wijgrid._field("resizer").inProgress()) {
							//return (wijField._canDrag() === true);

							if (wijField._canDrag() && wijgrid._trigger("columndragging", null, { drag: wijField.options })) {
								wijgrid._trigger("columndragged", null, { drag: wijField.options });
								return true;
							}
						}
				...
				.droppable({
					...
					over: function (e, ui) {
						...
						wijField.element.data("thisDroppableWijField", _droppableWijField);
					},
			...
			function _getWijFieldInstance(draggable) {
				return draggable.data(draggable.data($.wijmo.c1basefield.prototype._data$prefix + "widgetName"));
			}

			// position: "left", "right", "center"
			function _showArrows(wijField, position) {
				_topArrow()
				.show()
				.position({
					my: "center",
